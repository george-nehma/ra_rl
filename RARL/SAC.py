...
from .model import GaussianPolicy, QNetwork, DeterministicPolicy, StepLRMargin, StepResetLR, QEnsemble
...
class SAC(object):
    def __init__(self, config, dimList, action_space, disturbance_space, n_workers: int = None,):
        ...
        self.critics = QEnsemble(config, dimList, action_space.shape[0], disturbance_space.shape[0], self.num_critics).to(self.device)
        self.critic_targets = QNetwork(config, dimList, action_space.shape[0], disturbance_space.shape[0]).to(self.device)
        hard_update(self.critic_targets, self.critics)

        self.critic_optim = optim.AdamW(self.critics.parameters(), lr=config.LR_C, weight_decay=1e-3)
        self.scheduler = optim.lr_scheduler.StepLR(
            self.critic_optim, step_size=self.LR_C_PERIOD, gamma=self.LR_C_DECAY
        )
        ...
        # Unified inference modules
        # self.Q_network = VectorizedEnsembleWrapper(self.critics_net)
        # self.Q_target  = VectorizedEnsembleWrapper(self.critic_targets_net)
    ...
    #   self.critics.cntUpdate = value

    # @property
    # def GAMMA(self):
    #     return self.critics.GAMMA

    # @GAMMA.setter
    # def GAMMA(self, value):
    #     self.critics.GAMMA = value
    ...
    def get_uncertainty(self, state_tensor, control, disturbance: torch.Tensor) -> dict:
        ...
        """
        q_stack = self.critics(state_tensor, control, disturbance)   # (K, B, A)
        q_stack = q_stack.max(dim=1).values
    ...
    def update(self, memory, batch_size, updates, batch=None):
        ...
        epistem_uncertainty = self.get_uncertainty(state, action, disturbance)["epistemic_uncertainty"]
        ...
        q_values = self.critics(state, action, disturbance)

        q_means = q_values.mean(dim=0)
        # q_max = q_means.max(dim=0).values
        qf1, qf2 = q_means[0,:], q_means[1,:]
        ...
        with torch.no_grad():
            ...
            qf1_next, qf2_next = self.critic_targets(state_nxt, next_action, next_disturb)
        ...
        next_q_value = ((1 - self.GAMMA) * terminal + (~done) * self.GAMMA * non_terminal) # (~done) ignores the terminal states 
        ...
        if updates % 4 == 0:
            ...
            q_pi_val = self.critics(state, pi_pro, pi_adv.detach())
            q_pi_max = q_pi_val.max(dim=1).values
            mean_qf_pi = q_pi_max.mean(dim=0)

            # Protagonist wants to *minimise* Q  → minimise  Q - α·H
            smoothness_loss = 0.002 * F.mse_loss(action_next, action)
            pro_loss = (mean_qf_pi + self.alpha_pro * log_pi_pro).mean() + smoothness_loss
            ...
            # max_qf_pi = torch.max(qf1_pi, qf2_pi)
            q_adv_val = self.critics(state, pi_pro.detach(), pi_adv)
            q_adv_max = q_adv_val.max(dim=1).values
            mean_adv_pi = q_adv_max.mean(dim=0)

            # The adversary feeds its action into the *same* critic but wants
            # to drive Q *up* → maximise  Q + α·H  (entropy regularised) -loss
            adv_loss = (-mean_adv_pi + self.alpha_adv * log_pi_adv).mean()
        ...
        if updates % self.target_update_interval == 0:
            soft_update(self.critic_targets, self.critics, self.tau)
        ...
        torch.save(
            {
                ...
                "critic_target_state_dict":     self.critic_targets.state_dict(),
    ...
    def load_checkpoint(self, modelIter, ckpt_path, evaluate=True):
        ...
        critic_ckpt_path = os.path.join(ckpt_path, "critic_model", "critic_{}.pt".format(modelIter))
        ...
        if ckpt_path is not None:
            ...
            self.critics.load_state_dict(torch.load(critic_ckpt_path, map_location=self.device))
            ...
            for net in [self.protagonist, self.adversary, self.critics]:
                getattr(net, mode)()
    ...
    def unpack_batch(self, batch):
        ...
        done = torch.tensor(batch.done, dtype=torch.bool).to(self.device)
